...
using Arguments = StringVector;

/**
 * @brief This is the function type that is used to interface with the Console
 * class.
 *
...
public:
   ...
   */
  explicit Console(String const &greeting);
   ...
   * If the command already existed, it overwrites the previous entry.
   *
   * @param commands a vector of commands to register
   */
  void registerCommands(const ConsoleCommandVector &commands);
   ...
   */
  StringVector getRegisteredCommands() const;
  ...
  /**
   * @brief This function executes an arbitrary string as if it was inserted via
   * stdin.
   *
  ...
  /**
   * @brief This function calls an external script and executes all commands
   * inside.
   *
   * This function stops execution as soon as any single command returns
   * something different from 0, be it a quit code or an error code.
   *
   * @param filename The pathname of the script.
   *
   * @return What the last command executed returned.
   */
  int executeFile(const String &filename);
...
private:
  ...
  /**
   * @brief This function saves the current state so that some other Console can
   * make use of the GNU readline facilities.
   */
  void saveState();
  /**
   * @brief This function reserves the use of the GNU readline facilities to the
   * calling Console instance.
   */
  void reserveConsole();
