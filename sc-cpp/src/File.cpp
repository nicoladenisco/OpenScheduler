...
File File::getParent() const
{
  ...
  if(idx == String::npos)
    throw FileException(format("Impossibile ottenere il parent di %s", thePath.c_str()));
...
bool File::endWith(const String& str) const
{
  return strEndWith(thePath, str);
}
...
void File::readStat()
{
  if(thePath.length() > 0)
  {
    ...
    if(!existFlg) memset(&si, 0, sizeof (si));
  }
  else
  {
    absolutePath = true;
...
int File::deepScanInternal(int dept,
  const File &parent, File::FuncScan f, void* args, int maxDept, FuncScanOrder order, File::FuncScanRetVal &retVal) const
{
  int count = 0;
...
int File::deepScanInternalDir(int dept,
  const File &parent, File::FuncScan f, void* args, int maxDept, FuncScanOrder order, File::FuncScanRetVal &retVal) const
{
  int count = 0;
      ...
      switch(retVal)
      {
        ...
        case SCAN_CONTINUE:
          if(maxDept == 0 || dept < maxDept)
          {
            ...
            count += deepScanInternal(dept + 1, target, f, args, maxDept, order, retVal);
          }
          break;
      }
      continue;
...
int File::deepScanInternalFile(int dept, const File &parent, File::FuncScan f, void* args, File::FuncScanRetVal &retVal) const
{
  int count = 0;
  DIR *ptDir = opendir(parent.thePath.c_str());
  if(!ptDir)
 ...
 * @param dest path destinazione (la directory deve esistere, il file verrà sovrascritto se esiste)
 * @param blkSize dimensione blocco di lettura/scrittura
 * @return numero byte copiati o -1 per errore (vedi errno)
 */
int File::copyFile(const String& source, const String& dest, int blkSize /*= 4096*/)
{
 ...
 */
int File::copyEntry(void* arg, int dept,
  const File &parent, const File &target, const String& name)
{
  ...
  else if(target.isFile() || target.isLink())
  {
    if(copyFile(target.thePath.c_str(), out.thePath.c_str(), blksize) == -1)
      throw FileException(format("Errore di IO copiando %s in %s", target.thePath.c_str(), out.thePath.c_str()));
  }
  ...
  if(destPath.isDirectory())
  {
    ...
    else if(isFile() || isLink())
    {
      ...
      if(copyFile(thePath.c_str(), out.thePath.c_str(), si.st_blksize) == -1)
        throw FileException(format("Errore di IO copiando %s in %s", thePath.c_str(), destPath.thePath.c_str()));
    }
    else throw FileException("Tipo di oggetto non copiabile.");
  }
  else
  {
    if(isFile() || isLink())
    {
      if(copyFile(thePath.c_str(), destPath.thePath.c_str(), si.st_blksize) == -1)
        throw FileException(format("Errore di IO copiando %s in %s", thePath.c_str(), destPath.thePath.c_str()));
    }
    else throw FileException("Tipo di oggetto non copiabile.");
  }
...
int File::renameTo(const File &destPath)
{
  return renameTo(destPath.thePath);
}
...
int File::mkdir(int mode /*= 0755*/)
{
  if(exist())
  {
    ...
    throw FileException(format("La path %s esiste ma non è una directory.", thePath.c_str()));
  }
...
File File::createTempFile(const String& prefix, const String& suffix, const File& dir)
{
...
int File::createUnixTempFile(File &fTemp, const String& prefix, const File& dir)
{
  String tmpName = dir.getAbsolutePath() + getPathSep() + prefix + "XXXXXX";
  char *ptTmpName = strdupa(tmpName.c_str());
...
int File::removeLockFile()
{
  ...
  if(fdLock <= 0)
    throw FileException("removeLockFile() va chiamata dopo createLockFile() sullo stesso oggetto");
 ...
 */
int File::writeData(const void *data, int len) const
{
  if(thePath.length() == 0)
    return -1;
 ...
 * @return numero di linee lette dal file (può essere maggiore degli elementi in linesRead)
 */
int File::readTextFile(StringVector &linesRead, bool removeBlank /*= true*/) const
{
 ...
 */
int File::writeTextFile(const StringVector &linesWrite, bool append /*= false*/)
{
